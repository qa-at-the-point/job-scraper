...
QA_ENGINEER = "https://www.indeed.com/jobs?q=Software+QA+Engineer"
...
def parse_job_results(page_source: str) -> List[Job]:
    ...
    for job in soup.find_all(attrs={"data-testid": "slider_item"}):
        ...
        location = job.find("div", class_="company_location").find("div", attrs={"data-testid": "text-location"}).get_text(strip=True)
        ...
        job_type_element = job.find("div", attrs={"data-testid": "attribute_snippet_testid"}, text=lambda x: x and "time" in x)
...
def scrape(py: Pylenium, search_url: str = QA_ENGINEER) -> List[Job]:
    py.visit(search_url)
    # TODO: Add pagination
